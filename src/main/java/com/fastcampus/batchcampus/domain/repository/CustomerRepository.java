...
public interface CustomerRepository {
    List<Customer> findAll(Pageable pageable); // 모든 고객 정보를 페이지네이션하여 반환
    Customer findById(Long id); // ID로 고객 정보를 찾아 반환

    // CustomerRepository의 가짜 구현체: 테스트나 개발 시 사용
    class Fake implements CustomerRepository {
        ...
        public Customer findById(Long id) {
            // 모든 고객 데이터를 스트림으로 변환하여 ID로 필터링 후 반환
            return Stream.of(
                new Customer(0L, "A사", "A@company.com"),
                new Customer(1L, "B사", "B@company.com"),
                new Customer(2L, "C사", "C@company.com"),
                ...
                new Customer(18L, "S사", "S@company.com"),
                new Customer(19L, "T사", "T@company.com")
            ).filter(it -> it.getId().equals(id)) // ID로 필터링
            .findFirst() // 첫 번째 요소를 찾고
